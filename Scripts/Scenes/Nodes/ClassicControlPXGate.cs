...
[GlobalClass, Tool]
public partial class ClassicControlPXGate : GraphNode, ISaveableGate, ICompileableGateWithBits, IColorableGate
{

	// Called when the node enters the scene tree for the first time.
	public override void _Ready()
	{
		Title = "Controlled Pauli X Gate    ";
		SetSlots();
	}
    ...
    public LazyMatrix compile(int QBitCount, Array<int> ForQBits)
    {
		...
		var bit = ForQBits[0];
		if(bit == 0) return GateBuilder.Identity(QBitCount - 1) ^ GateBuilder.Hadamard(1);
		else if(bit == QBitCount - 1) return GateBuilder.Hadamard(1) ^ GateBuilder.Identity(QBitCount - 1);
		else return GateBuilder.Identity(QBitCount - bit - 1) ^ GateBuilder.Hadamard(1) ^ GateBuilder.Identity(bit);
    }
    ...
    public LazyMatrix compile(int QBitCount, Array<int> ForQBits, Array<int> classicBits, ClassicControlBitMatrix.GetMatrixIndexByBits getter)
    {
        if(ForQBits.Count != 1) throw new ArgumentException("Compiling for an invalid number of QBits");
        var bit = ForQBits[0];
        var Matrix = new ClassicControlBitMatrix(new IMatrix[] {GateBuilder.Identity(1), GateBuilder.PauliX()}, classicBits.ToArray(), getter);
